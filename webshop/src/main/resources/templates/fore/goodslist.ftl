	...
	<div class="products">	 
		<div class="container">
			...
			<div>
				...
				<#else >
					<a class='btn btn-info'  href="/fore/goodsList?pageNo=${page.pageNumber-1}&id=${id}" >上一页</a>
				</#if>
				...
				<#else>
					<a class='btn btn-info' href="/fore/goodsList?pageNo=${page.pageNumber+1}&id=${id}" >下一页</a>
				</#if>
				<#if page.pageNumber==page.totalPage>
					<a class='btn btn-info' disabled >尾页</a>
				<#else>
					<a class='btn btn-info' href="/fore/goodsList?pageNo=${page.totalPage}&id=${id}" >尾页</a>
				</#if>


<input type='text' class='form-control' style='display:inline;width:60px;' value=''/><a class='btn btn-info' href='javascript:void(0);' onclick='location.href="/fore/goodsList?id=${id}&pageNo="+(this.previousSibling.value)'>GO</a>
				</div>
</div>
		</div>
	</div>
