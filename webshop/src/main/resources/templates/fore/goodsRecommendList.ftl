	...
	<div class="products">	 
		<div class="container">
			...
			<div>
				...
				<#else>
					<a class='btn btn-info'  href="/fore/goodsRecommendList?pageNumber=1&typeId=${typeId}" >首页</a>
				</#if>
				<#if page.pageNumber==1>
					<a class='btn btn-info' disabled >上一页</a>
				<#else >
					<a class='btn btn-info'  href="/fore/goodsRecommendList?pageNumber=${page.pageNumber-1}&typeId=${typeId}" >上一页</a>
				</#if>
				...
				<#else>
					<a class='btn btn-info' href="/fore/goodsRecommendList?pageNumber=${page.pageNumber+1}&typeId=${typeId}" >下一页</a>
				</#if>
				<#if page.pageNumber==page.totalPage>
					<a class='btn btn-info' disabled >尾页</a>
				<#else>
					<a class='btn btn-info' href="/fore/goodsRecommendList?pageNumber=${page.totalPage}&typeId=${typeId}" >尾页</a>
				</#if>


<input type='text' class='form-control' style='display:inline;width:60px;' value=''/><a class='btn btn-info' href='javascript:void(0);' onclick='location.href="/fore/goodsRecommendList?typeId=${typeId}&pageNumber="+(this.previousSibling.value)'>GO</a>
				</div>
</div>
		</div>
	</div>
